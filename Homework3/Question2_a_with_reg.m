...
lamda = 10; % Regularisation factor
weights = inv(r'*r+lamda*eye(size(r)))*TrLabel';

% Test
testIdx = find(test_classlabel==1 | test_classlabel==5); % select classes 1, 2
TeLabel = zeros(size(test_classlabel)); % 1x59
...
xlabel('Thresholds');
ylabel('Accuracy');
title(strcat('Accuracy against Threshold (Regularization = ', " ", num2str(lamda), ")"))